...
namespace gui
{
    CentralPanel::CentralPanel (std::shared_ptr<juce::AudioProcessorValueTreeState> parameters,
        ...
        : juce::Component(), mClipMeter (parameters, levelMetersPack, ticks),
          mControlPanel (parameters, levelMetersPack), mLinkingPanel (parameters),
          mAnalyserComponent (parameters, levelMetersPack), mScalingSwitch (ticks)
    {
    ...
    void CentralPanel::resized()
    {
        ...
        grid.performLayout (localBounds.withTrimmedTop (toRemoveFromTop)
        ...
        mScalingSwitch.setBounds (localBounds.withWidth (localBounds.proportionOfWidth (0.076f))
