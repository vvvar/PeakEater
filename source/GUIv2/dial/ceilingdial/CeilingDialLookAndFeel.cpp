...
namespace gui
{
    ...
    CeilingDialLookAndFeel::CeilingDialLookAndFeel (std::shared_ptr<pe::dsp::LevelMeter<float>> inputMeter,
        ...
        : mInputMeter (inputMeter), mClippingMeter (clippingMeter), mRange (-36.0f, 0.0f, 0.1f, 1.9f), mSmoothedInput (inputMeter->getDecibels()), mSmoothedClipping (clippingMeter->getDecibels())
    {
