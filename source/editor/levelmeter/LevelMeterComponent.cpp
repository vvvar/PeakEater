...
LevelMeterComponent::LevelMeterComponent(std::shared_ptr<processor::LevelMeter<float>> levelMeter, std::shared_ptr<Ticks> ticks,
...
LevelMeterComponent::~LevelMeterComponent() { setLookAndFeel(nullptr); }
...
void LevelMeterComponent::paint(juce::Graphics& g) {
    ...
    juce::ColourGradient gradient(colourscheme::ForegroundPrimary, gradientPoint1, colourscheme::ForegroundSecondary, gradientPoint2,
                                  false);
    ...
    auto const valueTextSize =
        calculateTextSize(getTopLevelComponent()->getBounds().getWidth(), getTopLevelComponent()->getBounds().getHeight());
    g.setFont(valueTextSize);
    g.setColour(dbValueTextColour);
    g.drawText(std::to_string(static_cast<int>(dB)) + "dB", 0.0f, static_cast<int>(padding), static_cast<int>(width),
               static_cast<int>(height), juce::Justification::centredTop, true);
    auto const textSize =
    ...
    g.drawText(mName, 0.0f, 0.0f, static_cast<int>(width), static_cast<int>(height - padding), juce::Justification::centredBottom, true);
}
...
void LevelMeterComponent::onTimerTick() { repaint(); }
}  // namespace gui
}  // namespace pe