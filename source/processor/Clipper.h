...
class ClippingBlock {
   public:
    ...
    void setClippingType(ClippingType const type) {
        for (auto& clipper : clippers) {
            clipper.setClippingType(type);
        }
    }

    void setDryWetProportion(SampleType const proportion) {
        for (auto& clipper : clippers) {
            clipper.setDryWetProportion(proportion);
        }
    }

    void setOversamplingFactor(size_t const newOversamplingFactor) { oversamplingFactor = newOversamplingFactor; }

   private:
    size_t oversamplingFactor = 0;
    std::array<Clipper<SampleType>, 6> clippers{Clipper<SampleType>{0}, Clipper<SampleType>{1}, Clipper<SampleType>{2},
                                                Clipper<SampleType>{3}, Clipper<SampleType>{4}, Clipper<SampleType>{5}};
};
}  // namespace pe::processor
