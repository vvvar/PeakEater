...
class OversampledClipper : public juce::dsp::ProcessorBase {
   public:
    ...
    //==============================================================================
    OversampledClipper(const unsigned int factor = 1) noexcept : oversampler(2, factor, Oversampling::filterHalfBandFIREquiripple, true) {}
    ...
    void prepare(const ProcessorSpec& spec) noexcept override {
        ...
        *preFilter.state = *juce::dsp::IIR::Coefficients<T>::makeLowPass(
            oversampledSpec.sampleRate, calculateCutoff(oversampledSpec.sampleRate), calculateQ(oversampler.factorOversampling));
   ...
   private:
    ...
    unsigned int calculateCutoff(double sampleRate) const noexcept { return static_cast<unsigned int>((sampleRate / 2) * 0.98f); }
